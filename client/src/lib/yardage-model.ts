...
export interface EnvironmentalConditions {
  ...
  elevationChange: number; // feet (positive = uphill)
...
export interface YardageResult {
  ...
  aimOffset: string | null; // e.g., "Aim 5 yards right"
}

const DEFAULT_PDF = 2.5; // Default Personal Distance Factor
const BASELINE_TEMP = 70; // °F
...
export function recommendClub(
  ...
  // One club more (longer)
  if (bestIdx > 0) {
    alternatives.push(clubDistances[bestIdx - 1]);
  }
  // One club less (shorter)
