...
export function ClubEditor({ clubs, onChange }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-[1fr_80px_40px] gap-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider px-1">
        <span>Club</span>
        <span>Loft (°)</span>
        <span></span>
      </div>
      {clubs.map((club) => (
        <div key={club.id} className="grid grid-cols-[1fr_80px_40px] gap-2 items-center">
          <Input
            value={club.name}
